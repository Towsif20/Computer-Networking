...
public class NetworkLayerServer {
    ...
    public static synchronized void DVR(int startingRouterId)
    {
        ...
        while (convergence)
        {
            ...
            int size = r.getNeighborRouterIDs().size();

            for (int i=0;i<size;i++)
            {
                int id = r.getNeighborRouterIDs().get(i);

                Router neighbor = routerMap.get(id);

                if (neighbor.getState())
                {
                    boolean result = neighbor.sfupdateRoutingTable(r);
                    if (result)
                        flag = true;
                }
            }
            ...
            for (Map.Entry<Integer, Router> entry : routerMap.entrySet())
            {
                ...
                size = r.getNeighborRouterIDs().size();

                for (int i=0;i<size;i++)
                {
                    int neighborid = r.getNeighborRouterIDs().get(i);

                    Router neighbor = routerMap.get(neighborid);
    ...
    public static synchronized void simpleDVR(int startingRouterId)
    {
        ...
        while (convergence)
        {
            ...
            for (int i=0;i<size;i++)
            {
                ...
                if (neighbor.getState())
                {
                    boolean result = neighbor.updateRoutingTable(r);
                    if (result)
                        flag = true;
                }
            }
