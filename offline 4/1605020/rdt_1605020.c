...
void CRC(struct frm frame, int flag)
{
    ...
    for (i = 0; i < 4; i++)
    {
        ...
        for (j = 7; j >= 0; j--)
        {
            input_bit_string[index] = extract(taken, j) + '0';
            index++;
        }
    }
    ...
    for (i = 0; i < 32; i++)
    {
        ...
        else
        {
            for (j = 0; j < length; j++)
            {
                temp[j] = generator_polynomial[j];
            }
        }

        for (j = length - 1; j > 0; j--)
        {
            if (taker_from_beginning[j] == temp[j])
            {
                remainder[j - 1] = '0';
            }
            else
        ...
        if (i != 31)
        {
            remainder[length - 1] = data_transmitted[i + length];
        }
        else
        {
...
void sendAcknowledgmentFrameA(struct frm frame)
{
    ...
    ack_frame_A.checksum = checkSum(ack_frame_A);
...
void sendAcknowledgmentFrameB(struct frm frame)
{
    ...
    ack_frame_B.checksum = checkSum(ack_frame_B);
...
void A_output(struct pkt packet)
{
    ...
    frame_A.checksum = checkSum(frame_A);
...
void B_output(struct pkt packet)
{
    ...
    frame_B.checksum = checkSum(frame_B);
